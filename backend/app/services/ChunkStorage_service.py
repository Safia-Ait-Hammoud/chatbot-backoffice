from models.chunk_model import ChildModel, ChildPayload,ParentModel, ParentChildMetadata
from repositories.chunk_repository import ChunkRepository
from services.embedding_service import EmbeddingService
from schemas.chunks_storage_response import ChunksStorageResponse
...
class ChunkStorageService:
    ...
    async def chunks_storage(
     ...
     #parent
        ...
        result_parents =  await self.chunk_repository.create_manyParents(parents_to_create)
        if not result_parents:
            raise HttpException(
                status_code=500,
                detail="Erreur lors de la création des parents",
            )
        ...
        contents = [
            child["content"]
            for child, _, _ in flat_children
        ]
        ...
        result = await self.chunk_repository.create_many_children(project_id, children_to_create)
    ...
    async def delete_doc_chunks(self, project_id: str, document_id: str) -> bool:
        return await self.chunk_repository.delete_doc_chunks(project_id, document_id)